...
function loadDraftData() {
    $.ajax({
        ...
        success: function (response) {
            ...
            var referNames = receiverListData
                .filter(receiver => receiver.receiver_type === 1)
                .map(receiver => receiver.receiver_name + " <" + receiver.receiver_email + ">")
                .join(", ");
            ...
            var contentWithDivider = 
                ...
                "<p><strong>받는사람: </strong>" + receiverNames + "</p>" +
                "<p><strong>참조: </strong>" + referNames + "</p>" +
            ...
            if (senderDto) {
                var senderAsReceiver = {
                    name: senderDto.sender_name,
                    email: senderDto.sender_email,
                    type: 0, // 받는 사람 유형
...
/* 수신자 및 참조자 설정 */
function updateReceiverAndRefer(receiverListData) {
    if (!receiverListData || receiverListData.length === 0) return;

    receiverListData.forEach(receiver => {
        var newEntry = {
            name: receiver.name || receiver.receiver_name,
            email: receiver.email || receiver.receiver_email,
            type: receiver.type || receiver.receiver_type,
            emp_idx: receiver.emp_idx || receiver.receiver_idx
        };

        if (newEntry.type === 0) {
            // 받는 사람
            addReceiverOrRefer(newEntry, "receiver-list");
        } else if (newEntry.type === 1) {
            // 참조자
            addReceiverOrRefer(newEntry, "refer-list");
        }
    });
...
function renderFileList() {
    ...
    formData.getAll('files').forEach((file, i) => {
        var $fileDiv = $("<div>").addClass("filebox").attr("id", "file" + i);
        var $fileContent = $("<span>").addClass("file-content").text(file.name);
        var $deleteBtn = $("<i>").addClass("far fa-minus-square delete-icon").on("click", function () {
            deleteFile(i);
        });
...
function addFile() {
    ...
    let duplicateFiles = Array.from(files).filter(file => 
	    filesArr.some(existingFile => 
	        ...
	        existingFile.lastModified === file.lastModified // 추가
	    )
	);


    if (duplicateFiles.length > 0) {
    	modal.showAlert("이미 추가된 파일이 있습니다.");
        return;
    }
    ...
    if (files.length > remainFileCnt) {
    	modal.showAlert("첨부파일은 최대 " + maxFileCnt + "개까지 가능합니다.");
        return;
    }
